...
; (function (window) {
	...
	UIProgressButton.prototype._initEvents = function () {
		...
		this.button.addEventListener('click', function () {
			//通过正则表达式去判断手机号的合法性
			let iPhone = /^((0\d{2,3}-\d{7,8})|(1[3584]\d{9}))$/
			if (!$("#inpnt").val()) {
				layer.msg('请输入你的手机号才可以提交哦！');
			} else if (!iPhone.test($("#inpnt").val())) {
				layer.msg('当前手机号非法,请查正后重新输入');
			} else {
				//调用动画回调函数
				self._submit();
				setTimeout(function () {
					let sub = $(".submit .progress-button button")
					if (sub.prop("disabled") == true) {
						layer.msg('提交成功！请等待老师与您联系');
						setTimeout(function () {
							$("#inpnt").val("")
						}, 3000)
					}
